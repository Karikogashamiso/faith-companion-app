...
-- ===== 20260617050000_web_push.sql =====
-- Web push: store browser push subscriptions + make reminders deliverable when
-- the app is closed (timezone + per-day dedupe). Idempotent.
...
-- ===== 20260617060000_ai_aux_metering.sql =====
-- Close the AI cost bypass: meter explainChapter/dailyDevotional generations
-- against a separate generous daily counter (companions unlimited). Idempotent.
...
-- ===== 20260617080000_admin_funnel_metrics.sql =====
-- Admin funnel metrics RPC for the in-app dashboard (self-gated to admins).
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'cta_by_location', (
      ...
      FROM (
        SELECT COALESCE(props->>'location', 'unknown') AS loc, count(DISTINCT anon_id) AS c
        FROM public.landing_events
        WHERE event = 'cta_click' AND created_at > v_since GROUP BY 1
      ) s
    )
  ) INTO v_result
  FROM public.landing_events WHERE created_at > v_since;
  v_result := COALESCE(v_result, '{}'::jsonb) || jsonb_build_object(
    'signups', (SELECT count(DISTINCT props->>'anon_id') FROM public.analytics_events
                WHERE event = 'landing_conversion' AND created_at > v_since),
    'paywall_views', (SELECT count(*) FROM public.analytics_events
